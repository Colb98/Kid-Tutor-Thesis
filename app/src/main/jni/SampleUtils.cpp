...
void
SampleUtils::setIdentityMatrix(float* matrix)
{
    float identity[16] = {0};
    ...
    float *matrix)
{
    ...
    for (i = 0; i < 3; i++)
    {
        matrix[i * 4 + (i + 1) % 3] = static_cast<float>(u[(i + 2) % 3] * s);
        matrix[i * 4 + (i + 2) % 3] = static_cast<float>(-u[(i + 1) % 3] * s);
    }
...
float
SampleUtils::getMatrixDeterminate(float *m)
{
    return m[12] * m[9] * m[6]
        * m[3] - m[8] * m[13]
        * m[6] * m[3] - m[12]
...
void
SampleUtils::inverseMatrix(float *m, float *result)
{
    float temp[16];
            
    temp[0] = m[6] * m[11] * m[13]
        - m[7] * m[10] * m[13]
        + m[7] * m[9] * m[14]
        - m[5] * m[11] * m[14]
        - m[6] * m[9] * m[15]
...
void
SampleUtils::transposeMatrix(float *m, float *result)
{
    float temp[16];
